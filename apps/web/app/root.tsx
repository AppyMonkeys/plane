...
export const links: LinksFunction = () => [
  ...
  // Only one <link rel="manifest"> is honored by browsers; manifest.json has
  // correctly-sized icons (site.webmanifest.json's are a single 1024x1024
  // image mislabeled at multiple sizes) so it's the one kept.
...
export function HydrateFallback() {
  // The theme isn't known until after hydration (it depends on localStorage/media
  // queries, which the server can't see), so the first client render must match the
  // server's markup exactly — the spinner only appears once mounted is set post-hydration.
